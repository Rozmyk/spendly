...
export default function DashboardLayout() {
  ...
  useEffect(() => { refresh(); fetch(`${apiUrl}/categories`, { credentials: "include" }).then((response) => response.ok ? response.json() : []).then(setCategories).catch(() => setCategories([])); fetch(`${apiUrl}/financial-summary`, { credentials: "include" }).then((response) => response.ok ? response.json() : null).then(setSummary).catch(() => setSummary(null)); }, [refresh]);
