...
const workspaceNavigation = [{ label: "Overview", href: "/" }, { label: "Expenses", href: "/expenses" }, { label: "Analytics", href: "/analytics" }, { label: "Budgets", href: "/budgets" }, { label: "Settings" }];

export default function SidebarNavigation() {
  const pathname = usePathname();
  const itemSx = (active: boolean) => ({ justifyContent: "flex-start", minHeight: 42, px: 1.25, borderRadius: "var(--radius-sm)", color: active ? "var(--color-accent-ink)" : "var(--color-ink-2)", bgcolor: active ? "var(--color-accent)" : "transparent", textTransform: "none", fontSize: "var(--text-sm)", fontWeight: active ? 800 : 700, "&:hover": { bgcolor: active ? "var(--color-accent)" : "var(--color-paper-2)" } });
